...
const Event = ({event}:{event:Event|null}) => {
  ...
  const { title,date, resume, userId, images, subEvents=[] , swimmingType} = event;
  const LABES ={
    "25m":"Pool 25m",
    "50m":"Pool 50m",
  ...
  return (
    <div>
      ...
      <div>
        <h3 className="font-bold text-lg">The events <span className='text-sm font-normal'>{`(${subEvents.length||0})`}</span></h3>
